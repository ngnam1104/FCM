...
# ============================================================================
# TEMPORAL MATCHING - Sử dụng cho câu hỏi "when"
# ============================================================================

def check_temporal_answer(rag_answer: str, expected: str, session_date: str) -> tuple[bool, float]:
    """
    Kiểm tra câu trả lời temporal với session date.
    
    Ví dụ:
    - Expected: "2022", RAG: "last year", Session: "8 May, 2023" → 2023-1=2022 ✓
    - Expected: "7 May 2023", RAG: "yesterday", Session: "8 May, 2023" → 8-1=7 ✓
    
    Returns:
        (is_correct, confidence)
    """
    if not rag_answer or not session_date:
        return False, 0.0
    ...
    day_match = re.search(r'(\d{1,2})\s+(\w+)', session_date)
    if day_match:
        session_day = int(day_match.group(1))
        month_name = day_match.group(2).lower()
        session_month = months.get(month_name, 0)
    
    # Check for "last year" pattern
    if 'last year' in norm_answer or 'previous year' in norm_answer:
        try:
            expected_year = int(norm_expected)
            if session_year and (session_year - 1) == expected_year:
                return True, 0.95
        except ValueError:
            pass
    
    # Check for "yesterday" pattern
    if 'yesterday' in norm_answer or 'day before' in norm_answer:
        # Try to parse expected as a date
        exp_day_match = re.search(r'(\d{1,2})\s+(\w+)', norm_expected)
        if exp_day_match and session_day and session_month:
            exp_day = int(exp_day_match.group(1))
            exp_month_name = exp_day_match.group(2).lower()
            exp_month = months.get(exp_month_name, 0)
            
            # Check if expected is yesterday of session date
            if exp_month == session_month and (session_day - 1) == exp_day:
                return True, 0.95
    
    # Check for year directly mentioned in answer
    if session_year:
        # If answer contains year-1 and expected is year-1
        try:
            expected_year = int(norm_expected)
            if str(expected_year) in norm_answer:
                return True, 0.9
            if str(session_year - 1) in norm_answer and expected_year == session_year - 1:
                return True, 0.9
        except ValueError:
            pass
...
# ============================================================================
# CATEGORY DEFINITIONS - LoCoMo Question Categories
# ============================================================================

QUESTION_CATEGORIES = {
    1: "Single-hop Factual",      # Câu hỏi đơn giản về facts
    2: "Temporal Reasoning",       # Câu hỏi về thời gian (when)
    3: "Multi-hop Reasoning",      # Câu hỏi phức tạp, cần suy luận nhiều bước
    4: "Inference",                # Câu hỏi suy luận
    5: "Adversarial",              # Câu hỏi đánh lừa
}

def get_category_name(category_id: int) -> str:
    """Get category name from ID"""
    return QUESTION_CATEGORIES.get(category_id, f"Category {category_id}")
...
def load_locomo_dataset(path: str = "dataset/locomo10.json", max_samples: int = 10, 
    ...
    for item in raw_data[:max_samples]:
        ...
        for qa in item.get("qa", [])[:5]:
            ...
            # Get category from LoCoMo dataset
            category = qa.get("category", 1)  # Default to category 1
...
def check_answer_fuzzy(retrieved: str, expected: str, question: str, session_date: str = None) -> tuple[bool, float]:
    """
    ...
    2. Temporal matching for "when" questions (score = 0.95)
    3. Synonym/Semantic match (score = 0.85)
    4. Keyword overlap (score = 0.5-0.9)
    5. Topic relevance (score = 0.3-0.7)
    """
    if not retrieved or not retrieved.strip():
        return False, 0.0
    ...
    # Strategy 2: Temporal matching for "when" questions
    if session_date:
        temporal_match, temporal_score = check_temporal_answer(retrieved, expected, session_date)
        if temporal_match:
            return True, temporal_score
    
    # Strategy 3: Synonym/Semantic match
...
def run_locomo_comparison(dataset_path: str = "dataset/locomo10.json", verbose: bool = True, 
    ...
    # Count total questions and by category
    ...
    for cat_id, count in sorted(category_counts.items()):
        ...
        print(f"   • {cat_name} (Cat {cat_id}): {count}")
    ...
    # Collect category stats from data
    qa_categories = {}
    idx = 0
    for item in data:
        for qa in item.get("qa", []):
            if idx < len(v1_details):
                qa_categories[idx] = qa.get("category", 1)
            idx += 1
    ...
    for idx, (v1_d, v2_d) in enumerate(zip(v1_details, v2_details)):
        cat = qa_categories.get(idx, 1)
        
        if cat not in v1_by_cat:
            v1_by_cat[cat] = {"correct": 0, "total": 0}
            v2_by_cat[cat] = {"correct": 0, "total": 0}
        
        v1_by_cat[cat]["total"] += 1
        v2_by_cat[cat]["total"] += 1
        
        if v1_d["correct"]:
            v1_by_cat[cat]["correct"] += 1
        if v2_d["correct"]:
            v2_by_cat[cat]["correct"] += 1
    
    print(f"\n{'Category':<25} {'V1 Accuracy':^15} {'V2 Accuracy':^15} {'Winner':^10}")
    print("-" * 70)
    
    for cat_id in sorted(v1_by_cat.keys()):
        cat_name = get_category_name(cat_id)
        v1_acc = v1_by_cat[cat_id]["correct"] / max(1, v1_by_cat[cat_id]["total"])
        v2_acc = v2_by_cat[cat_id]["correct"] / max(1, v2_by_cat[cat_id]["total"])
        ...
        v1_str = f"{v1_by_cat[cat_id]['correct']}/{v1_by_cat[cat_id]['total']} ({v1_acc:.0%})"
        v2_str = f"{v2_by_cat[cat_id]['correct']}/{v2_by_cat[cat_id]['total']} ({v2_acc:.0%})"
        print(f"{cat_name:<25} {v1_str:^15} {v2_str:^15} {winner:^10}")
    
    # Detailed comparison
    print_header("DETAILED COMPARISON", "-")
    
    print(f"\n{'#':<4} {'Sample':<8} {'Cat':^5} {'V1':^6} {'V2':^6} {'Question':<40}")
    print("-" * 80)
    
    for idx, (v1_d, v2_d) in enumerate(zip(v1_details, v2_details), 1):
        v1_status = "✅" if v1_d["correct"] else "❌"
        v2_status = "✅" if v2_d["correct"] else "❌"
        cat = qa_categories.get(idx - 1, 1)
        question = v1_d["question"][:37] + "..." if len(v1_d["question"]) > 40 else v1_d["question"]
        print(f"{idx:<4} {v1_d['sample_id']:<8} {cat:^5} {v1_status:^6} {v2_status:^6} {question:<40}")
